...
export const trainUpload = async (req, res, next) => {
  try {
    ...
    if (isTrainUploaded)
      return res.status(401).json({ message: "Train already uploaded" });
    ...
    res
      .status(200)
      .json({ message: "Train and stations updated successfully" });
  } catch (error) {
    console.error("\n\n\n\n\nError:", error);
    next(error);
  }
...
const getTotalSeats = async (trainId, seatClass) => {
  try {
    ...
    const totalSeats =
      train.coach_structure[seatClass].total_coaches *
      train.coach_structure[seatClass].seats_per_coach;
...
export const bookTicket = async (req, res, next) => {
  try {
    ...
    if (!availableSeat) {
      return next(
        errorHandler(401, { text: "seats not available", availableSeat })
      );
    }
...
export const payment = async (req, res, next) => {
  ...
  try {
    ...
    console.log("payment uploaded to db");
    ...
    const updateBookings = pnrList.map(async (pnr) => {
      ...
      if (updatedBooking.status !== "Success") {
        ...
        throw new Error(
          `Booking with PNR ${pnr} did not have 'Success' status after update.`
        );
      }
    ...
    // Return 500 for server errors to distinguish from auth issues
    next(errorHandler(500, e || "Payment processing failed"));
  }
};
...
export const getPnr = async (req, res, next) => {
  ...
  try {
    ...
    if (!bookingDetails)
      return next(errorHandler(401, "PNR number is not valid"));
    ...
    trainDetails.stations = trainDetails.stations.filter(
      (station) =>
        station.name.trim().toLowerCase() ===
          bookingDetails.from_station.name.trim().toLowerCase() ||
        station.name.trim().toLowerCase() ===
          bookingDetails.to_station.name.trim().toLowerCase()
    );
...
export const getMyBooking = async (req, res, next) => {
  try {
    ...
    if (!pnrList || pnrList.length === 0) {
      return res
        .status(200)
        .json({ success: true, message: "No bookings found", bookings: [] });
    }
...
export const getTrainInfo = async (req, res, next) => {
  try {
    ...
    if (!trains.length) {
      return res
        .status(404)
        .json({ success: false, message: "No train found" });
    }
