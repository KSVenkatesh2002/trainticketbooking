...
const Payment = () => {
    const {currentTrainList, bookingTrain,sourceName,destinationName, finalDate, selectedClass, passengerInfo, contactInfo} = useSelector((state) => state.train);
    ...
    async function bookTicket(method){
    ...
    return (
        <div className='bg-gray-200 h-full p-3 w-full flex flex-col justify-center  overflow-hidden md:flex-row md:justify-between'>
            ...
            <div className='w-full flex flex-col justify-center items-center border py-5 rounded-4xl md:flex-col md:flex-wrap  md:w-7/11'>
                ...
                <div className='w-full flex flex-col mt-2 mb-5 py-3 md:flex-row md:justify-around'>
                    ...
                    <div className='flex flex-col items-center mt-2 mb-5 py-3 w-full bg-white shadow-md rounded-lg md:w-7/12'>
                        {/* train header */}
                        <div>
                            <div>
                    ...
                    <table className='w-full border-collapse bg-white shadow-md rounded-lg border border-gray-400 text-center  md:w-4/12'>
                        <tr>
                            ...
                            <td className='border border-gray-300 px-2 p-1'>{t.fare[selectedClass]}</td>
                        </tr>
                        <tr>
                            <th className='border border-gray-300 w-7/10 p-2 bg-orange-400'>No. ofpassenger</th>
                            <td className='border border-gray-300 px-2 p-1'>{passengerInfo.length}</td>
                        </tr>
                        <tr>
                            <th className='border border-gray-300 w-7/10 p-2 bg-orange-400'>Total Fare</th>
                            <td className='border border-gray-300 px-2 p-1'>{t.fare[selectedClass]*passengerInfo.length}</td>
                        </tr>
                    </table>
                ...
                <div className='w-full flex justify-between items-center bg-white p-3 mb-2 rounded-lg shadow-md md:w-7/10 '>
                    <table className='w-full border-collapse border border-gray-400 text-center'>
                        <tr>
                            ...
                            <td className='border border-gray-300 px-2 p-1'>{contactInfo.name}</td>
                        </tr>
                        
                        <tr>
                            <th className='border border-gray-300 w-4/18 p-2 bg-orange-400'>Email</th>
                            <td className='border border-gray-300 px-2 p-1'>{contactInfo.email}</td>
                        </tr>
                        <tr>
                            <th className='border border-gray-300 w-4/18 p-2 bg-orange-400'>Phone</th>
                            <td className='border border-gray-300 px-2 p-1'>{contactInfo.phoneno}</td>
                        </tr> 
                    </table>
            ...
            <div className='w-full h-fit flex flex-col justify-center items-center border py-5 rounded-4xl md:flex-row md:flex-wrap md:w-3/11 md:items-start'>
                ...
                <div className='w-full bg-gray-300 rounded-t-2xl ' onClick={()=>setPayMethodCard(!payMethodCard)}>
                    <span className={`w-1/2 inline-block ${payMethodCard && 'bg-orange-400 border-b-6 rounded-tl-2xl'} text-center p-2`}> Card</span>
                    <span className={`w-1/2 inline-block ${!payMethodCard && 'bg-orange-400 border-b-6 rounded-tr-2xl'} text-center p-2`}>BHMI / UPI</span>
                </div>
                ...
                <div className='w-full bg-white rounded-b-lg p-2 py-5 shadow-2xl '>
                    ...
                    <form className='w-full flex flex-wrap justify-center'>
                        ...
                        <button className='w-1/2 bg-blue-500 p-2 rounded-lg text-white mt-3' onClick={()=>{bookTicket('card')}}>Pay Now</button>
                    </form>}

                    {/* upi option */}
                    {!payMethodCard &&
                    <ol className='bg-white px-1 '>
                        <li className='border border-gray-400 rounded-md m-1 p-1'>
                        ...
                        {upiId && <li className='text-center w-full'><button className='w-1/2 bg-blue-500 p-2 rounded-lg text-white mt-3' onClick={()=>{bookTicket('upi')}}>Pay Now</button></li>}
                    </ol>}
                </div>
            </div>
        </div>
